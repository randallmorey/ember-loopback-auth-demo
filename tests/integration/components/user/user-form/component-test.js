...
test('it triggers the external saveUser action on submit', function (assert) {
  run(() => {
    const store = this.container.lookup('service:store');
    let user = store.createRecord('user', {});
    this.set('user', user);
    this.set('saveUser', (savedUser) => {
      assert.ok(savedUser);
    ...
    this.render(hbs`{{user/user-form user=user saveUser=(action saveUser)}}`);
    // fill out the form and force an onchange
    this.$('[name="email"]').val('test@foo.com');
    this.$('[name="email"]').change();
    this.$('[name="emailConfirmation"]').val('test@foo.com');
